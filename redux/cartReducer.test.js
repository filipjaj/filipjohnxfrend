...
describe("addToCart", () => {
  test("checks that quantity is 1 when new item is added to cart", () => {
    const state = [];
    const product = {
      id: 1,
      name: "Cargo Trouser",
  ...
  test("decrements correct objects stock", () => {
    const state = [
      {
        id: 1,
        name: "Cargo Trouser",
    ...
    const newState = cartReducer(state, addToCart(product));
    expect(
      newState.find((p) => p.cartId === product.cartId).variants.stock
    ).toBe(0);
  });
  test("doesn't do anything when stock is 0", () => {
    const state = [
      {
        id: 1,
        name: "Cargo Trouser",
...
describe("increment", () => {
  test("increments correct object", () => {
    ...
    deepFreeze(state);
    const newState = cartReducer(state, incrementQuantity(product.cartId));
    expect(newState.find((p) => p.cartId === product.cartId).quantity).toBe(2);
  });

  test("decrements stock", () => {
    const state = [
      {
        id: 1,
        name: "Cargo Trouser",
